...
const resultado = await axios.post(process.env.NEXT_PUBLIC_API_URL+'/api/login',{
  userID: userid,
  password: password
}).then(async(res) => {
...
if(valorCookie !== token2fa){
  ...
  Cookies.set('TwoFA', token2fa, { path: '/2FA'})
        ...
        router.push('/2FA')
        const DoisFatores = await axios.get(process.env.NEXT_PUBLIC_API_URL+'/api/2fa/'+id)
      }
    }
  }

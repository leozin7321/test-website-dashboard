...
export async function getServerSideProps({ req, res }) {
  ...
  const userApi = await fetch(process.env.NEXT_PUBLIC_API_URL+'/infobot/'+decoded)
  .then(r => r.json())
  
  const infosApi = await fetch(process.env.NEXT_PUBLIC_API_URL+'/api/infogeral/'+decoded)
  .then(r => r.json())
  .catch(console.error)
  return {
